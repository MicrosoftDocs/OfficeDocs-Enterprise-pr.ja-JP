---
title: "いつでも使用可能"
ms.author: josephd
author: JoeDavies-MSFT
manager: laurawi
ms.date: 9/1/2017
ms.audience: ITPro
ms.topic: concetpual
ms.prod: office-online-server
localization_priority: Normal
ms.collection:
- Ent_O365
- Ent_O365_Top
ms.custom:
- DecEntMigration
- Ent_Architecture
ms.assetid: c27d9a45-92f4-4d14-affc-9a4c459910be
description: "概要: Microsoft クラウド サービスに組み込まれる一連のクラウド ストレージ オプションの詳細について説明します。"
---

# いつでも使用可能

 **概要:** Microsoft クラウド サービスに組み込まれる一連のクラウド ストレージ オプションの詳細について説明します。
  
"いつでも使用可能" なストレージ ソリューション:
  
- 事前にパッケージ化されたソリューションの即時使用
    
- すぐに使用できる、既存のサービスとのバンドル
    
- 限定カスタマイズの提供
    
次のセクションでは、"いつでも使用可能" ストレージのそれぞれのオプションの詳細について説明します。
  
## Office 365 の SharePoint Online

### 機能

- ファイルは安全に保護され、信頼でき、複数のデバイスでどこからでもアクセス可能
    
- コンテンツは検索によって簡単に検出
    
- ワークフローとアイテム保持ポリシーにより、簡単にコンテンツを使用および管理
    
- コンテンツは API またはブラウザーから、あるいはオフラインでアクセス可能
    
### 一般的な使用目的

- 企業または組織のポータル
    
- チーム単位の共同作業
    
- エンタープライズ検索
    
### ストレージの主要なシナリオ

- 共同作業
    
- 会社の通信
    
- ビデオの管理
    
### リソース

追加情報については、[こちら](https://support.office.com/article/SharePoint-Online-and-OneDrive-for-Business-software-boundaries-and-limits-8f34ff47-b749-408b-abc0-b605e1f6d498)をクリックしてください。
  
コストの情報については、[こちら](https://products.office.com/business/compare-office-365-for-business-plans)をクリックしてください。
  
## Exchange Online in Office 365

### 機能

- メールボックスは安全に保護され、信頼でき、複数のデバイスでどこからでもアクセス可能
    
- 検索によってメールボックスの内容を検出可能
    
### 一般的な使用目的

- 企業のメール ストレージ
    
### ストレージの主要なシナリオ

- 共同作業
    
### リソース

追加情報については、[こちら](https://technet.microsoft.com/library/exchange-online-limits.aspx)をクリックしてください。
  
コストの情報については、[こちら](https://products.office.com/business/compare-office-365-for-business-plans)をクリックしてください。
  
## OneDrive for Business

### 機能

- ストレージは安全に保護され、信頼でき、複数のデバイスでどこからでもアクセス可能
    
- コンテンツはユーザーによる検索で簡単に検出
    
- コンテンツは API またはブラウザーから、あるいはオフラインでアクセス可能
    
- 各ユーザーに 1 TB のストレージ
    
### 一般的な使用目的

- 個々のユーザーのドキュメント ストア
    
- ドキュメントのアドホック コラボレーション
    
### ストレージの主要なシナリオ

- ファイルのバックアップ
    
### リソース

追加情報については、[こちら](https://support.office.com/article/What-is-OneDrive-for-Business-187f90af-056f-47c0-9656-cc0ddca7fdc2)をクリックしてください。
  
コストの情報については、[こちら](https://products.office.com/business/compare-office-365-for-business-plans)をクリックしてください。
  
## Visual Studio Team Services

### 機能

- 組み込みの冗長性あり
    
- Team Foundation Server に基づく
    
- バックログを作成し、スプリントで作業し、変更とバグを追跡
    
- ソース コードを格納するための最適な方法 (SharePoint Online へのドキュメントの格納を推奨)
    
### 一般的な使用目的

- ソース コード リポジトリ
    
### ストレージの主要なシナリオ

- ソース コードの管理
    
### リソース

追加情報については、[こちら](https://www.visualstudio.com/products/what-is-visual-studio-online-vs.aspx)をクリックしてください。
  
コストの情報については、[こちら](https://www.visualstudio.com/products/visual-studio-online-pricing-vs.aspx)をクリックしてください。
  
## Yammer のファイル共有

### 機能

- プレミアム ネットワークへの各アップロード ファイル サイズを 5 GB に制限
    
- アップロードできるファイル数は無制限
    
- 迅速な情報共有やディスカッションに最適
    
- ブラウザー経由でアクセス可能
    
### 一般的な使用目的

- エンタープライズ ソーシャル
    
- 会話やトピックに関連するドキュメントやリンクの共有
    
### ストレージの主要なシナリオ

- 共同作業
    
### リソース

追加情報については、[こちら](https://products.office.com/yammer/yammer-overview)をクリックしてください。
  
コストの情報については、[こちら](https://products.office.com/business/compare-office-365-for-business-plans)をクリックしてください。
  
## Dynamics 365

### 機能

- 10 GB のストレージを含むサブスクリプション
    
- プロフェッショナル USL が 20 増えるごとに、5 GB のストレージ容量を追加
    
- 顧客ごとに最大 5 TB のストレージ
    
### 一般的な使用目的

- カスタマー リレーションシップを格納して管理
    
### ストレージの主要なシナリオ

- 共同作業
    
### リソース

追加情報については、[こちら](http://www.microsoft.com/dynamics/default.aspx?DYNCRM-SEARCH?CR_CC=200470741&amp;WT.mc_ID=DynGB_en_us_SEM_GOOG&amp;DYNCRM-SEARCH&amp;WT.srch=1)をクリックしてください。
  
## Microsoft Intune

### 機能

- 展開するすべてのソフトウェアをパッケージ化し、Microsoft Intune クラウド ストレージにアップロード
    
- サブスクリプションには、20 GB のストレージと、次のようなオプションが含まれる。追加容量の購入、各種ソフトウェアおよびハードウェアのインベントリ レポート、ポリシー管理、モバイル アプリの公開、アラート、監視、99.99% のアップタイム SLA
    
### 一般的な使用目的

- ソフトウェア展開
    
### ストレージの主要なシナリオ

- ファイルのバックアップ
    
### リソース

追加情報については、[こちら](https://www.microsoft.com/en-us/server-cloud/products/microsoft-intune/)をクリックしてください。
  
コストの情報については、[こちら](https://www.microsoft.com/ja-jp/server-cloud/products/microsoft-intune/Purchasing.aspx)をクリックしてください。
  
## Azure Site Recovery

### 機能

- シンプルかつ自動化された保護、レプリケーション、および Azure へのリカバリ
    
- 継続的な正常性の監視と、調整されたリカバリ
    
- Managed Disks と Ubuntu 仮想マシン のサポート
    
- Windows Server 2016 とストレージ スペースのサポート
    
### 一般的な使用目的

- 2 つのデータ センター間、または Azure に対して、HYPER-V を使用する環境で実行されている仮想マシンのレプリケートが可能
    
- InMage Scout による VMware および物理環境の保護が可能
    
### ストレージの主要なシナリオ

- ファイルのバックアップ
    
### リソース

追加情報については、[こちら](http://azure.microsoft.com/services/site-recovery/?WT.srch=1&amp;WT.mc_ID=kn0iWCHA)をクリックしてください。
  
コストの情報については、[こちら](http://azure.microsoft.com/pricing/details/site-recovery/)をクリックしてください。
  
## Azure Backup

### 機能

- Windows Server および Windows Server Essentials から受信したデータのシームレスな保護
    
- System Center Data Protection Manager を含む
    
- データは Geo レプリケートされたストレージで保管される。そこでは、2 つの Azure データセンターにわたるデータのコピーを 6 部保管する
    
- 差分変更、頻度の定義、圧縮、暗号化、保存期間の延長、帯域幅調整をサポート
    
### 一般的な使用目的

- さまざまなオンプレミス (SharePoint、SQL、Exchange) と Azure (VM) ソースからのデータ保護
    
### ストレージの主要なシナリオ

- ファイルのバックアップ
    
### リソース

追加情報については、[こちら](http://azure.microsoft.com/services/backup/)をクリックしてください。
  
コストの情報については、[こちら](http://azure.microsoft.com/pricing/details/backup/)をクリックしてください。
  
## 次の手順

[いくらかのアセンブリが必要](some-assembly-required.md) クラウド ストレージ オプションを確認します。
  
## See Also

#### 

[エンタープライズ アーキテクトのための Microsoft クラウド ストレージ](microsoft-cloud-storage-for-enterprise-architects.md)
  
[Microsoft クラウド IT アーキテクチャのリソース](microsoft-cloud-it-architecture-resources.md)
#### 

[Microsoft's Enterprise Cloud Roadmap: Resources for IT Decision Makers](https://sway.com/FJ2xsyWtkJc2ta)

