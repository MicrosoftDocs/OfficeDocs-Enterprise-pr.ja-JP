...
**の概要:** ユーザー アカウントにライセンスを割り当てるし、同時に Office 365 の PowerShell を使用して特定のサービス プランを無効にする方法を説明します。
...
- 組織内のサブスクリプションは、 **AccountSkuId** \<"組織名">:\<サブスクリプション > 形式です。\<"組織名"> は、Office 365 に登録し、組織の一意では、指定した値です。\<サブスクリプション > 値は、特定のサブスクリプションの。たとえば、litwareinc:ENTERPRISEPACK、組織名は、litwareinc とサブスクリプション名は、ENTERPRISEPACK (Office 365 エンタープライズ E3)。
...
1 人のユーザーのユーザー アカウント、AccountSkuId、サービス プランの一覧を無効にして、説明のテキストを削除するユーザー プリンシパル名を記入し、\<と > 文字です。PowerShell コマンド プロンプトで、結果のコマンドを実行します。
