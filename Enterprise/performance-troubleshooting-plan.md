...
- 通話中のトレースで、使用している Office 365 サービスを見つけます。 以前にトラフィックが発生したことがないかめったにわからない場合、これは、パフォーマンスの問題を他のネットワークノイズから分離するための便利な手順です。 これを行うには、いくつかの方法があります。 テストの直前に、 _ping_または_psping_を特定のサービスの URL (またはなど) に対して使用でき `ping outlook.office365.com` `psping -4 microsoft-my.sharepoint.com:443` ます。 また、Netmon トレースに ping や PsPing がある場合は、そのプロセス名を使用すると簡単に見つけることができます。 これで、検索を開始する場所がわかります。
